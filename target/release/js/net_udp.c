...
napi_value js_net_udp_recvfrom(napi_env env, napi_callback_info info) {
    ...
    io_Result  frrr = net_udp_recvfrom( local_0, local_1, local_1_tail, local_3, local_3_tail, local_5);
    assert(status == napi_ok);
    return jsreturn;
}

napi_value jsGet_net_udp_Socket_ctx(napi_env env, napi_callback_info info) {
  napi_status status;
  ...
  assert(status == napi_ok);
  mem += sizeof(size_t);
  net_udp_Socket * obj = (net_udp_Socket*)mem;
...
napi_value jsGet_net_udp_Socket_impl_sendto(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_net_udp_Socket_impl_recvfrom(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_net_udp_Socket_impl_close(napi_env env, napi_callback_info info) {
  napi_status status;
...
void js_delete_net_udp_Socket(napi_env env, void *obj, void*hint) {
    free(obj);
}
napi_value js_new_net_udp_Socket(napi_env env, napi_callback_info info) {
    napi_status status;
    ...
    void *obj = malloc(sizeof(size_t) + sizeof_net_udp_Socket);
    memset(obj, 0, sizeof(size_t) + sizeof_net_udp_Socket);
    ...
    *((size_t *)obj) = tail;
    ...
    status = napi_wrap(env,
            jsthis,
            obj,
            js_delete_net_udp_Socket, // destructor
            0,  // finalize_hint
            0
    );
    assert(status == napi_ok);
...
void js_register_net_udp_Socket (napi_env env, napi_value exports) {
    napi_property_descriptor properties[] = {
        { "close", 0, js_net_udp_close, 0, 0, 0, napi_default, 0 },
        { "recvfrom", 0, js_net_udp_recvfrom, 0, 0, 0, napi_default, 0 },
        { "udp", 0, js_net_udp, 0, 0, 0, napi_default, 0 },
        { "sendto", 0, js_net_udp_sendto, 0, 0, 0, napi_default, 0 },
        { "ctx", 0, 0, jsGet_net_udp_Socket_ctx, 0, 0, napi_default, 0},
        { "impl_sendto", 0, 0, jsGet_net_udp_Socket_impl_sendto, 0, 0, napi_default, 0},
        { "impl_recvfrom", 0, 0, jsGet_net_udp_Socket_impl_recvfrom, 0, 0, napi_default, 0},
        { "impl_close", 0, 0, jsGet_net_udp_Socket_impl_close, 0, 0, napi_default, 0},
    };
    napi_value cc;
    napi_status status = napi_define_class(env, "Socket", NAPI_AUTO_LENGTH, js_new_net_udp_Socket, 0, 8, properties, &cc);
    assert(status == napi_ok);
...
napi_value js_net_udp_sendto(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_5 == 0 || status != napi_ok) {
        ...
        local_5 = tttt_local_5 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    io_Result  frrr = net_udp_sendto( local_0, local_1, local_1_tail, local_3, local_3_tail, local_5);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_net_udp_os_new(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_4 == 0 || status != napi_ok) {
        ...
        local_4 = tttt_local_4 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    net_udp_Socket  frrr = net_udp_os_new( local_0, local_1, local_1_tail, local_3, local_4);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_net_udp_close(napi_env env, napi_callback_info info) {
    ...
    assert(status == napi_ok);
                    ...
                    napi_value jsreturn = 0;
    net_udp_close( local_0);
    return jsreturn;
}


napi_value js_net_udp_Init(napi_env env, napi_value exports)
{
    js_register_net_udp_Socket(env, exports);
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "recvfrom", NAPI_AUTO_LENGTH, js_net_udp_recvfrom, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "recvfrom", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "sendto", NAPI_AUTO_LENGTH, js_net_udp_sendto, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "sendto", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "os_new", NAPI_AUTO_LENGTH, js_net_udp_os_new, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "os_new", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "close", NAPI_AUTO_LENGTH, js_net_udp_close, 0, &ff);
    assert(status == napi_ok);
