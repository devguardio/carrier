...
napi_value js_slice_slice_eq_cstr(napi_env env, napi_callback_info info) {
    ...
    char * local_1;
                    ...
                    napi_value jsreturn = 0;
    bool  frrr = slice_slice_eq_cstr( local_0, local_1);
    status = napi_create_uint32(env, frrr, &jsreturn);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_slice_slice_borrow(napi_env env, napi_callback_info info) {
    ...
    assert(status == napi_ok);
                    ...
                    napi_value jsreturn = 0;
    slice_slice_Slice * frrr = slice_slice_borrow( local_0);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_slice_slice_make(napi_env env, napi_callback_info info) {
    ...
    status = napi_get_value_uint32(env, argv[1], (uint32_t*)&local_2);
                    ...
                    napi_value jsreturn = 0;
    slice_slice_make( local_0, local_1, local_2);
    return jsreturn;
}
...
napi_value jsGet_slice_slice_Slice_size(napi_env env, napi_callback_info info) {
  ...
  assert(status == napi_ok);
  mem += sizeof(size_t);
  slice_slice_Slice * obj = (slice_slice_Slice*)mem;
...
napi_value jsGet_slice_slice_Slice_mem(napi_env env, napi_callback_info info) {
  napi_status status;
...
void js_delete_slice_slice_Slice(napi_env env, void *obj, void*hint) {
    free(obj);
}
napi_value js_new_slice_slice_Slice(napi_env env, napi_callback_info info) {
    napi_status status;
    ...
    void *obj = malloc(sizeof(size_t) + sizeof_slice_slice_Slice);
    memset(obj, 0, sizeof(size_t) + sizeof_slice_slice_Slice);
    ...
    *((size_t *)obj) = tail;
    ...
    status = napi_wrap(env,
            jsthis,
            obj,
            js_delete_slice_slice_Slice, // destructor
            0,  // finalize_hint
            0
    );
    assert(status == napi_ok);
...
void js_register_slice_slice_Slice (napi_env env, napi_value exports) {
    ...
    napi_value cc;
    napi_status status = napi_define_class(env, "Slice", NAPI_AUTO_LENGTH, js_new_slice_slice_Slice, 0, 7, properties, &cc);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "Slice", cc);
    assert(status == napi_ok);
}
...
napi_value js_slice_slice_Init(napi_env env, napi_value exports)
{
