...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_cmd_shell.h"
...
napi_value js_carrier_cmd_shell_shell_usage(napi_env env, napi_callback_info info);
...
napi_value js_carrier_cmd_shell_cmd(napi_env env, napi_callback_info info) {
    ...
    int  frrr = carrier_cmd_shell_cmd( local_0, local_1);
...
napi_value js_carrier_cmd_shell_shell_usage(napi_env env, napi_callback_info info) {
    ...
    napi_value jsreturn = 0;
    int  frrr = carrier_cmd_shell_shell_usage();
    status = napi_create_int32(env, frrr, &jsreturn);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_carrier_cmd_shell_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "cmd", NAPI_AUTO_LENGTH, js_carrier_cmd_shell_cmd, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "cmd", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "shell_usage", NAPI_AUTO_LENGTH, js_carrier_cmd_shell_shell_usage, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "shell_usage", ff);
    assert(status == napi_ok);
    return exports;
}
