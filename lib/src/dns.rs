...
impl DnsRecord {
    pub fn to_signed_txt(&self, sign: &Secret) -> String {
        let txt = format!("carrier={} {} {} {}", self.epoch, self.priority, self.addr, self.x.to_string());
    ...
    pub fn from_signed_txt<S: AsRef<str>>(s: S) -> Option<Self> {
        ...
        Some(DnsRecord { priority, addr, x, epoch })
    }

}
pub fn resolve(domain: &str)
    -> impl Future<Item = Vec<DnsRecord>, Error = Error>
{
