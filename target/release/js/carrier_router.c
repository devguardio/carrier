...
napi_value js_carrier_router_cleanup_dead_broker_route(napi_env env, napi_callback_info info);
...
napi_value js_carrier_router_make(napi_env env, napi_callback_info info);
...
napi_value jsGet_carrier_router_Router_pkt(napi_env env, napi_callback_info info) {
  napi_status status;
  ...
  assert(status == napi_ok);
  mem += sizeof(size_t);
  carrier_router_Router * obj = (carrier_router_Router*)mem;
...
napi_value jsSet_carrier_router_Router_pkt(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_router_Router_udp4(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsSet_carrier_router_Router_udp4(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_router_Router_udp6(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsSet_carrier_router_Router_udp6(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_router_Router_channels(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsSet_carrier_router_Router_channels(napi_env env, napi_callback_info info) {
  napi_status status;
...
void js_delete_carrier_router_Router(napi_env env, void *obj, void*hint) {
    free(obj);
}
napi_value js_new_carrier_router_Router(napi_env env, napi_callback_info info) {
    napi_status status;
    ...
    void *obj = malloc(sizeof(size_t) + sizeof_carrier_router_Router);
    memset(obj, 0, sizeof(size_t) + sizeof_carrier_router_Router);
    ...
    *((size_t *)obj) = tail;
    ...
    status = napi_wrap(env,
            jsthis,
            obj,
            js_delete_carrier_router_Router, // destructor
            0,  // finalize_hint
            0
    );
    assert(status == napi_ok);
...
void js_register_carrier_router_Router (napi_env env, napi_value exports) {
    napi_property_descriptor properties[] = {
        { "close", 0, js_carrier_router_close, 0, 0, 0, napi_default, 0 },
        { "push", 0, js_carrier_router_push, 0, 0, 0, napi_default, 0 },
        { "poll", 0, js_carrier_router_poll, 0, 0, 0, napi_default, 0 },
        { "disconnect", 0, js_carrier_router_disconnect, 0, 0, 0, napi_default, 0 },
        { "next_channel", 0, js_carrier_router_next_channel, 0, 0, 0, napi_default, 0 },
        { "pkt", 0, 0, jsGet_carrier_router_Router_pkt, jsSet_carrier_router_Router_pkt, 0, napi_default, 0},
        { "udp4", 0, 0, jsGet_carrier_router_Router_udp4, jsSet_carrier_router_Router_udp4, 0, napi_default, 0},
        { "udp6", 0, 0, jsGet_carrier_router_Router_udp6, jsSet_carrier_router_Router_udp6, 0, napi_default, 0},
        { "channels", 0, 0, jsGet_carrier_router_Router_channels, jsSet_carrier_router_Router_channels, 0, napi_default, 0},
    };
    napi_value cc;
    napi_status status = napi_define_class(env, "Router", NAPI_AUTO_LENGTH, js_new_carrier_router_Router, 0, 9, properties, &cc);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "Router", cc);
    assert(status == napi_ok);
}
...
napi_value js_carrier_router_poll(napi_env env, napi_callback_info info) {
    ...
    io_Result  frrr = carrier_router_poll( local_0, local_1, local_1_tail, local_3);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_carrier_router_disconnect(napi_env env, napi_callback_info info) {
    ...
    status = napi_get_value_bigint_uint64(env, argv[1], (uint64_t*)&local_3);
                    ...
                    napi_value jsreturn = 0;
    carrier_router_disconnect( local_0, local_1, local_1_tail, local_3);
    return jsreturn;
}
...
napi_value js_carrier_router_push(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_1 == 0 || status != napi_ok) {
        ...
        local_1 = tttt_local_1 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    bool  frrr = carrier_router_push( local_0, local_1, local_1_tail);
...
napi_value js_carrier_router_make(napi_env env, napi_callback_info info) {
    ...
    net_udp_Socket  local_1;

                    if (status != napi_ok) {
                        napi_throw_type_error(env, 0, "2'th arg requires type ::net::udp::Socket");
                        return 0;
                    }
    ...
    net_udp_Socket  local_2;
                    ...
                    napi_value jsreturn = 0;
    carrier_router_Router  frrr = carrier_router_make( local_0, local_1, local_2);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_carrier_router_next_channel(napi_env env, napi_callback_info info) {
    ...
    assert(status == napi_ok);
                    ...
                    napi_value jsreturn = 0;
    carrier_channel_Channel * frrr = carrier_router_next_channel( local_0);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_carrier_router_cleanup_dead_broker_route(napi_env env, napi_callback_info info) {
    ...
    uint64_t  local_1;
    status = napi_get_value_bigint_uint64(env, argv[1], (uint64_t*)&local_1);
                    ...
                    napi_value jsreturn = 0;
    carrier_router_cleanup_dead_broker_route( local_0, local_1);
    return jsreturn;
}
...
napi_value js_carrier_router_read_routing_key(napi_env env, napi_callback_info info) {
    ...
    carrier_router_Direction * local_1;
    ...
    if (tttt_local_1 == 0 || status != napi_ok) {
        ...
        local_1 = tttt_local_1 + sizeof(size_t*);
                    ...
                    if (status != napi_ok) {
                        napi_throw_type_error(env, 0, "2'th arg requires type ::carrier::router::Direction*");
                        return 0;
                    }
                    napi_value jsreturn = 0;
    uint64_t  frrr = carrier_router_read_routing_key( local_0, local_1);
...
napi_value js_carrier_router_Init(napi_env env, napi_value exports)
{
    js_register_carrier_router_Router(env, exports);
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "poll", NAPI_AUTO_LENGTH, js_carrier_router_poll, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "poll", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "close", NAPI_AUTO_LENGTH, js_carrier_router_close, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "close", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "disconnect", NAPI_AUTO_LENGTH, js_carrier_router_disconnect, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "disconnect", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "push", NAPI_AUTO_LENGTH, js_carrier_router_push, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "push", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "make", NAPI_AUTO_LENGTH, js_carrier_router_make, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "make", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "next_channel", NAPI_AUTO_LENGTH, js_carrier_router_next_channel, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "next_channel", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "cleanup_dead_broker_route", NAPI_AUTO_LENGTH, js_carrier_router_cleanup_dead_broker_route, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "cleanup_dead_broker_route", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "read_routing_key", NAPI_AUTO_LENGTH, js_carrier_router_read_routing_key, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "read_routing_key", ff);
    assert(status == napi_ok);
    return exports;
}
