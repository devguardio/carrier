...
// repeaer loop that runs until Shutdown to avoid race conditions
...
func (self *Channel) Open(path string) (*Stream, error) {
    ...
    self.synx <- &synx {
        ...
        open: &synxOpen {
            ...
            opt:    openStreamOptions{
                ...
                OnPoll: func() *[]byte {
                    select {
                        case msg := <- tx: return &msg;
                        default: return nil;
                    }
                },
                OnFragmented : func(frags uint32) {
                    frag_expected   = frags;
                },
            },
        },
    ...
    select {
        case headers := <- await_headers: {
            status, _ := strconv.Atoi(string(headers[":status"]));
            if status < 200 || status >= 300 {
                em := "status: " + string(headers[":status"]) + " : " + string(headers[":error"]);
                //TODO close chan
