...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_cmd_config_net.h"
...
napi_value js_carrier_cmd_config_net_run_self_net_get(napi_env env, napi_callback_info info);
napi_value js_carrier_cmd_config_net_run_self_net_join(napi_env env, napi_callback_info info);
...
napi_value js_carrier_cmd_config_net_run_remote_net_get(napi_env env, napi_callback_info info);
...
napi_value js_carrier_cmd_config_net_run_remote_net_join(napi_env env, napi_callback_info info);
...
napi_value js_carrier_cmd_config_net_run_self_net_join(napi_env env, napi_callback_info info) {
    ...
    carrier_endpoint_Endpoint * local_2;
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    if (status != napi_ok) {
                        napi_throw_type_error(env, 0, "3'th arg requires type ::carrier::identity::Secret*");
                        return 0;
                    }
                    napi_value jsreturn = 0;
    carrier_cmd_config_net_run_self_net_join( local_0, local_0_tail, local_2, local_3);
    return jsreturn;
}
...
napi_value js_carrier_cmd_config_net_on_close(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_1 == 0 || status != napi_ok) {
        ...
        local_1 = tttt_local_1 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_cmd_config_net_on_close( local_0, local_1, local_1_tail);
    return jsreturn;
}
...
napi_value js_carrier_cmd_config_net_run_remote_net_get(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_cmd_config_net_run_remote_net_get( local_0, local_0_tail, local_2, local_3);
    return jsreturn;
}
...
napi_value js_carrier_cmd_config_net_run_self_net_get(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_2 == 0 || status != napi_ok) {
        ...
        local_2 = tttt_local_2 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_cmd_config_net_run_self_net_get( local_0, local_0_tail, local_2);
    return jsreturn;
}
...
napi_value js_carrier_cmd_config_net_on_stream(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    bool  frrr = carrier_cmd_config_net_on_stream( local_0, local_1, local_1_tail, local_3);
...
napi_value js_carrier_cmd_config_net_run_remote_net_join(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_4 == 0 || status != napi_ok) {
        ...
        local_4 = tttt_local_4 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_cmd_config_net_run_remote_net_join( local_0, local_0_tail, local_2, local_3, local_4);
    return jsreturn;
}



napi_value js_carrier_cmd_config_net_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "run_self_net_join", NAPI_AUTO_LENGTH, js_carrier_cmd_config_net_run_self_net_join, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "run_self_net_join", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "on_close", NAPI_AUTO_LENGTH, js_carrier_cmd_config_net_on_close, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "on_close", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "run_remote_net_get", NAPI_AUTO_LENGTH, js_carrier_cmd_config_net_run_remote_net_get, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "run_remote_net_get", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "run_self_net_get", NAPI_AUTO_LENGTH, js_carrier_cmd_config_net_run_self_net_get, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "run_self_net_get", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "on_stream", NAPI_AUTO_LENGTH, js_carrier_cmd_config_net_on_stream, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "on_stream", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "run_remote_net_join", NAPI_AUTO_LENGTH, js_carrier_cmd_config_net_run_remote_net_join, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "run_remote_net_join", ff);
    assert(status == napi_ok);
    return exports;
}
