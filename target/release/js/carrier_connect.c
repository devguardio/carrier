...
napi_value js_carrier_connect_on_stream(napi_env env, napi_callback_info info) {
    ...
    bool  frrr = carrier_connect_on_stream( local_0, local_1, local_1_tail, local_3);
...
napi_value js_carrier_connect_start(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_4 == 0 || status != napi_ok) {
        ...
        local_4 = tttt_local_4 + sizeof(size_t*);
                    ...
                    if (status != napi_ok) {
                        napi_throw_type_error(env, 0, "3'th arg requires type ::carrier::identity::Identity*");
                        return 0;
                    }
                    napi_value jsreturn = 0;
    carrier_connect_start( local_0, local_1, local_1_tail, local_3, local_4);
    return jsreturn;
}

napi_value jsGet_carrier_connect_Connect_target(napi_env env, napi_callback_info info) {
  napi_status status;
  ...
  assert(status == napi_ok);
  mem += sizeof(size_t);
  carrier_connect_Connect * obj = (carrier_connect_Connect*)mem;
...
napi_value jsGet_carrier_connect_Connect_principal(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_connect_Connect_initiator(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_connect_Connect_initiator_time(napi_env env, napi_callback_info info) {
  napi_status status;
  ...
  napi_value value;
    status = napi_create_bigint_uint64(env, obj->initiator_time, &value);
...
napi_value jsGet_carrier_connect_Connect_initiator_pkt(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsSet_carrier_connect_Connect_initiator_pkt(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_connect_Connect_on_disconnect(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_connect_Connect_on_connect(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_connect_Connect_user1(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_connect_Connect_user2(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsSet_carrier_connect_Connect_user2(napi_env env, napi_callback_info info) {
  napi_status status;
  ...
  carrier_connect_Connect * obj = (carrier_connect_Connect*)mem;
    ...
    status = napi_get_value_uint32(env, value, (uint32_t*)&obj->user2);
...
void js_delete_carrier_connect_Connect(napi_env env, void *obj, void*hint) {
    free(obj);
}
napi_value js_new_carrier_connect_Connect(napi_env env, napi_callback_info info) {
    napi_status status;
    ...
    void *obj = malloc(sizeof(size_t) + sizeof_carrier_connect_Connect);
    memset(obj, 0, sizeof(size_t) + sizeof_carrier_connect_Connect);
    ...
    *((size_t *)obj) = tail;
    ...
    status = napi_wrap(env,
            jsthis,
            obj,
            js_delete_carrier_connect_Connect, // destructor
            0,  // finalize_hint
            0
    );
    assert(status == napi_ok);
...
void js_register_carrier_connect_Connect (napi_env env, napi_value exports) {
    napi_property_descriptor properties[] = {
        { "start", 0, js_carrier_connect_start, 0, 0, 0, napi_default, 0 },
        { "target", 0, 0, jsGet_carrier_connect_Connect_target, 0, 0, napi_default, 0},
        { "principal", 0, 0, jsGet_carrier_connect_Connect_principal, 0, 0, napi_default, 0},
        { "initiator", 0, 0, jsGet_carrier_connect_Connect_initiator, 0, 0, napi_default, 0},
        { "initiator_time", 0, 0, jsGet_carrier_connect_Connect_initiator_time, 0, 0, napi_default, 0},
        { "initiator_pkt", 0, 0, jsGet_carrier_connect_Connect_initiator_pkt, jsSet_carrier_connect_Connect_initiator_pkt, 0, napi_default, 0},
        { "on_disconnect", 0, 0, jsGet_carrier_connect_Connect_on_disconnect, 0, 0, napi_default, 0},
        { "on_connect", 0, 0, jsGet_carrier_connect_Connect_on_connect, 0, 0, napi_default, 0},
        { "user1", 0, 0, jsGet_carrier_connect_Connect_user1, 0, 0, napi_default, 0},
        { "user2", 0, 0, jsGet_carrier_connect_Connect_user2, jsSet_carrier_connect_Connect_user2, 0, napi_default, 0},
    };
    napi_value cc;
    napi_status status = napi_define_class(env, "Connect", NAPI_AUTO_LENGTH, js_new_carrier_connect_Connect, 0, 10, properties, &cc);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "Connect", cc);
    assert(status == napi_ok);
}
...
napi_value js_carrier_connect_on_close(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_1 == 0 || status != napi_ok) {
        ...
        local_1 = tttt_local_1 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_connect_on_close( local_0, local_1, local_1_tail);
    return jsreturn;
}


napi_value js_carrier_connect_Init(napi_env env, napi_value exports)
{
    js_register_carrier_connect_Connect(env, exports);
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "on_stream", NAPI_AUTO_LENGTH, js_carrier_connect_on_stream, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "on_stream", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "start", NAPI_AUTO_LENGTH, js_carrier_connect_start, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "start", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "on_close", NAPI_AUTO_LENGTH, js_carrier_connect_on_close, 0, &ff);
    assert(status == napi_ok);
