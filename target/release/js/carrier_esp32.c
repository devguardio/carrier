...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_esp32.h"
...
napi_value js_carrier_esp32_reboot_stream_open(napi_env env, napi_callback_info info);
...
napi_value js_carrier_esp32_close_later(napi_env env, napi_callback_info info);
...
napi_value js_carrier_esp32_check_err(napi_env env, napi_callback_info info);
napi_value js_carrier_sha256_init(napi_env env, napi_callback_info info);
napi_value js_carrier_esp32_ota_stream_open(napi_env env, napi_callback_info info);
...
napi_value js_carrier_esp32_ota_stream_stream(napi_env env, napi_callback_info info);
...
napi_value js_carrier_esp32_ota_stream_close(napi_env env, napi_callback_info info);
...
napi_value js_carrier_esp32_register_reboot(napi_env env, napi_callback_info info);
...
napi_value js_carrier_esp32_register_ota(napi_env env, napi_callback_info info);
...
napi_value js_carrier_esp32_close_later(napi_env env, napi_callback_info info) {
    ...
    carrier_esp32_close_later( local_0);
    return jsreturn;
}
...
napi_value js_carrier_esp32_ota_stream_open(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_esp32_ota_stream_open( local_0, local_1, local_1_tail, local_3);
    return jsreturn;
}
...
napi_value js_carrier_esp32_ota_stream_stream(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    bool  frrr = carrier_esp32_ota_stream_stream( local_0, local_1, local_1_tail, local_3);
...
napi_value js_carrier_esp32_reboot_stream_open(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_esp32_reboot_stream_open( local_0, local_1, local_1_tail, local_3);
    return jsreturn;
}
...
napi_value js_carrier_esp32_check_err(napi_env env, napi_callback_info info) {
    ...
    esp_err_t  local_2;
                    ...
                    if (status != napi_ok) {
                        napi_throw_type_error(env, 0, "1'th arg requires type ::ext::<esp_err.h>::esp_err_t");
                        return 0;
                    }
    ...
    status = napi_get_value_uint32(env, argv[3], (uint32_t*)&local_5);
                    ...
                    napi_value jsreturn = 0;
    bool  frrr = carrier_esp32_check_err( local_0, local_0_tail, local_2, local_3, local_4, local_5);
    status = napi_create_uint32(env, frrr, &jsreturn);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_carrier_esp32_ota_stream_close(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_1 == 0 || status != napi_ok) {
        ...
        local_1 = tttt_local_1 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_esp32_ota_stream_close( local_0, local_1, local_1_tail);
    return jsreturn;
}
...
napi_value js_carrier_esp32_register_reboot(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_0 == 0 || status != napi_ok) {
        ...
        local_0 = tttt_local_0 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_esp32_register_reboot( local_0);
    return jsreturn;
}
...
napi_value js_carrier_esp32_register_ota(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_0 == 0 || status != napi_ok) {
        ...
        local_0 = tttt_local_0 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_esp32_register_ota( local_0);
    return jsreturn;
}


napi_value js_carrier_esp32_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "close_later", NAPI_AUTO_LENGTH, js_carrier_esp32_close_later, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "close_later", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "ota_stream_open", NAPI_AUTO_LENGTH, js_carrier_esp32_ota_stream_open, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "ota_stream_open", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "ota_stream_stream", NAPI_AUTO_LENGTH, js_carrier_esp32_ota_stream_stream, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "ota_stream_stream", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "reboot_stream_open", NAPI_AUTO_LENGTH, js_carrier_esp32_reboot_stream_open, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "reboot_stream_open", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "check_err", NAPI_AUTO_LENGTH, js_carrier_esp32_check_err, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "check_err", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "ota_stream_close", NAPI_AUTO_LENGTH, js_carrier_esp32_ota_stream_close, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "ota_stream_close", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "register_reboot", NAPI_AUTO_LENGTH, js_carrier_esp32_register_reboot, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "register_reboot", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "register_ota", NAPI_AUTO_LENGTH, js_carrier_esp32_register_ota, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "register_ota", ff);
    assert(status == napi_ok);
    return exports;
}
