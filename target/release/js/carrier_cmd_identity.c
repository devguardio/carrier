...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_cmd_identity.h"
...
napi_value js_carrier_cmd_identity_cmd(napi_env env, napi_callback_info info) {
    ...
    int  frrr = carrier_cmd_identity_cmd( local_0, local_1);
...
napi_value js_carrier_cmd_identity_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "cmd", NAPI_AUTO_LENGTH, js_carrier_cmd_identity_cmd, 0, &ff);
    assert(status == napi_ok);
