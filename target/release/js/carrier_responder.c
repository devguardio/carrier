...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_responder.h"
...
napi_value js_carrier_responder_accept_insecure(napi_env env, napi_callback_info info) {
    ...
    slice_mut_slice_MutSlice * local_6;
    ...
    if (tttt_local_6 == 0 || status != napi_ok) {
        ...
        local_6 = tttt_local_6 + sizeof(size_t*);
                    ...
                    if (status != napi_ok) {
                        napi_throw_type_error(env, 0, "5'th arg requires type ::slice::mut_slice::MutSlice*");
                        return 0;
                    }
                    napi_value jsreturn = 0;
    carrier_responder_accept_insecure( local_0, local_1, local_1_tail, local_3, local_4, local_5, local_6);
    return jsreturn;
}


napi_value js_carrier_responder_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "accept_insecure", NAPI_AUTO_LENGTH, js_carrier_responder_accept_insecure, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "accept_insecure", ff);
    assert(status == napi_ok);
    return exports;
}
