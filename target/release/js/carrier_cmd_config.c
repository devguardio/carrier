...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_cmd_config.h"
...
napi_value js_carrier_cmd_config_on_result_stream(napi_env env, napi_callback_info info) {
    ...
    bool  frrr = carrier_cmd_config_on_result_stream( local_0, local_1, local_1_tail, local_3);
...
napi_value js_carrier_cmd_config_cmd(napi_env env, napi_callback_info info) {
    ...
    char ** local_1;
                    ...
                    napi_value jsreturn = 0;
    int  frrr = carrier_cmd_config_cmd( local_0, local_1);
...
napi_value js_carrier_cmd_config_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "on_result_stream", NAPI_AUTO_LENGTH, js_carrier_cmd_config_on_result_stream, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "on_result_stream", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "cmd", NAPI_AUTO_LENGTH, js_carrier_cmd_config_cmd, 0, &ff);
    assert(status == napi_ok);
