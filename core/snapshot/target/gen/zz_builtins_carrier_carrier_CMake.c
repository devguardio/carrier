#ifndef ZZ_EXPORT_HEADER___zz__builtins
#define ZZ_EXPORT_HEADER___zz__builtins
#include <stddef.h>
const __attribute__ ((unused)) size_t carrier_endpoint_InvalidBootstrap = 1;
const __attribute__ ((unused)) size_t io_NotImplemented = 2;
const __attribute__ ((unused)) size_t err_SystemError = 3;
const __attribute__ ((unused)) size_t hpack_decoder_Invalid = 4;
const __attribute__ ((unused)) size_t carrier_channel_InvalidFrame = 5;
const __attribute__ ((unused)) size_t err_OutOfTail = 6;
const __attribute__ ((unused)) size_t err_NotImplemented = 7;
const __attribute__ ((unused)) size_t io_Timeout = 8;
const __attribute__ ((unused)) size_t protonerf_Invalid = 9;
const __attribute__ ((unused)) size_t carrier_sync_Failed = 10;
const __attribute__ ((unused)) size_t carrier_identity_Invalid = 11;
const __attribute__ ((unused)) size_t carrier_cipher_EncryptionError = 12;
const __attribute__ ((unused)) size_t err_InvalidArgument = 13;
const __attribute__ ((unused)) size_t toml_ParseError = 14;
const __attribute__ ((unused)) size_t json_ParseError = 15;
const __attribute__ ((unused)) size_t carrier_router_OutOfOptions = 16;
const __attribute__ ((unused)) size_t carrier_router_Disconnected = 17;
size_t __attribute__ ((unused)) __zz_symbol_names_len = 18;
const char * __attribute__ ((unused)) __zz_symbol_names[] = {0,
    "carrier::endpoint::InvalidBootstrap",
    "io::NotImplemented",
    "err::SystemError",
    "hpack::decoder::Invalid",
    "carrier::channel::InvalidFrame",
    "err::OutOfTail",
    "err::NotImplemented",
    "io::Timeout",
    "protonerf::Invalid",
    "carrier::sync::Failed",
    "carrier::identity::Invalid",
    "carrier::cipher::EncryptionError",
    "err::InvalidArgument",
    "toml::ParseError",
    "json::ParseError",
    "carrier::router::OutOfOptions",
    "carrier::router::Disconnected",
};
#endif
