...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_pub_sysinfo.h"
...
napi_value js_carrier_pub_sysinfo_sensors_open(napi_env env, napi_callback_info info) {
    ...
    carrier_pub_sysinfo_sensors_open( local_0, local_1, local_1_tail, local_3);
    return jsreturn;
}
...
napi_value js_carrier_pub_sysinfo_sysinfo_open(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_pub_sysinfo_sysinfo_open( local_0, local_1, local_1_tail, local_3);
    return jsreturn;
}
...
napi_value js_carrier_pub_sysinfo_register(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_0 == 0 || status != napi_ok) {
        ...
        local_0 = tttt_local_0 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_pub_sysinfo_register( local_0);
    return jsreturn;
}


napi_value js_carrier_pub_sysinfo_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "sensors_open", NAPI_AUTO_LENGTH, js_carrier_pub_sysinfo_sensors_open, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "sensors_open", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "sysinfo_open", NAPI_AUTO_LENGTH, js_carrier_pub_sysinfo_sysinfo_open, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "sysinfo_open", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "register", NAPI_AUTO_LENGTH, js_carrier_pub_sysinfo_register, 0, &ff);
    assert(status == napi_ok);
