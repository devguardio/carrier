...
napi_value js_carrier_channel_transfer_from_symmetric(napi_env env, napi_callback_info info);
...
napi_value jsGet_carrier_channel_Transfer_version(napi_env env, napi_callback_info info) {
  napi_status status;
  ...
  assert(status == napi_ok);
  mem += sizeof(size_t);
  carrier_channel_Transfer * obj = (carrier_channel_Transfer*)mem;
...
napi_value jsGet_carrier_channel_Transfer_route(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_channel_Transfer_responder(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_channel_Transfer_c_mine(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsSet_carrier_channel_Transfer_c_mine(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_channel_Transfer_c_them(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsSet_carrier_channel_Transfer_c_them(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_channel_Transfer_rtt(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_channel_Transfer_counter_out(napi_env env, napi_callback_info info) {
  napi_status status;
...
napi_value jsGet_carrier_channel_Transfer_outgoing_acks_at(napi_env env, napi_callback_info info) {
  napi_status status;
...
void js_delete_carrier_channel_Transfer(napi_env env, void *obj, void*hint) {
    free(obj);
}
napi_value js_new_carrier_channel_Transfer(napi_env env, napi_callback_info info) {
    napi_status status;
    ...
    void *obj = malloc(sizeof(size_t) + sizeof_carrier_channel_Transfer);
    memset(obj, 0, sizeof(size_t) + sizeof_carrier_channel_Transfer);
    ...
    *((size_t *)obj) = tail;
    ...
    status = napi_wrap(env,
            jsthis,
            obj,
            js_delete_carrier_channel_Transfer, // destructor
            0,  // finalize_hint
            0
    );
    assert(status == napi_ok);
...
void js_register_carrier_channel_Transfer (napi_env env, napi_value exports) {
    napi_property_descriptor properties[] = {
        { "version", 0, 0, jsGet_carrier_channel_Transfer_version, 0, 0, napi_default, 0},
        { "route", 0, 0, jsGet_carrier_channel_Transfer_route, 0, 0, napi_default, 0},
        { "responder", 0, 0, jsGet_carrier_channel_Transfer_responder, 0, 0, napi_default, 0},
        { "c_mine", 0, 0, jsGet_carrier_channel_Transfer_c_mine, jsSet_carrier_channel_Transfer_c_mine, 0, napi_default, 0},
        { "c_them", 0, 0, jsGet_carrier_channel_Transfer_c_them, jsSet_carrier_channel_Transfer_c_them, 0, napi_default, 0},
        { "rtt", 0, 0, jsGet_carrier_channel_Transfer_rtt, 0, 0, napi_default, 0},
        { "counter_out", 0, 0, jsGet_carrier_channel_Transfer_counter_out, 0, 0, napi_default, 0},
        { "outgoing_acks_at", 0, 0, jsGet_carrier_channel_Transfer_outgoing_acks_at, 0, 0, napi_default, 0},
    };
    napi_value cc;
    napi_status status = napi_define_class(env, "Transfer", NAPI_AUTO_LENGTH, js_new_carrier_channel_Transfer, 0, 8, properties, &cc);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "Transfer", cc);
    assert(status == napi_ok);
}
...
napi_value js_carrier_channel_from_transfer(napi_env env, napi_callback_info info) {
    ...
    carrier_channel_Transfer  local_1;

                    if (status != napi_ok) {
                        napi_throw_type_error(env, 0, "1'th arg requires type ::carrier::channel::Transfer");
                        return 0;
                    }
                    napi_value jsreturn = 0;
    carrier_channel_from_transfer( local_0, local_1);
    return jsreturn;
}
...
void js_register_carrier_channel_Channel (napi_env env, napi_value exports) {
    napi_property_descriptor properties[] = {
        ...
        { "from_transfer", 0, js_carrier_channel_from_transfer, 0, 0, 0, napi_default, 0 },
    ...
    napi_value cc;
    napi_status status = napi_define_class(env, "Channel", NAPI_AUTO_LENGTH, js_new_carrier_channel_Channel, 0, 29, properties, &cc);
    assert(status == napi_ok);
...
napi_value js_carrier_channel_transfer_from_symmetric(napi_env env, napi_callback_info info) {
    ...
    status = napi_get_value_uint32(env, argv[3], (uint32_t*)&local_3);
                    ...
                    napi_value jsreturn = 0;
    carrier_channel_Transfer  frrr = carrier_channel_transfer_from_symmetric( local_0, local_1, local_2, local_3);
    assert(status == napi_ok);
    return jsreturn;
}
...
{
    js_register_carrier_channel_Transfer(env, exports);
    ...
    assert(status == napi_ok);
    status = napi_create_function(env, "from_transfer", NAPI_AUTO_LENGTH, js_carrier_channel_from_transfer, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "from_transfer", ff);
    assert(status == napi_ok);
    ...
    assert(status == napi_ok);
    status = napi_create_function(env, "transfer_from_symmetric", NAPI_AUTO_LENGTH, js_carrier_channel_transfer_from_symmetric, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "transfer_from_symmetric", ff);
    assert(status == napi_ok);
