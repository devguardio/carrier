...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_sft.h"
...
napi_value js_carrier_sft_sft_stream(napi_env env, napi_callback_info info) {
    ...
    bool  frrr = carrier_sft_sft_stream( local_0, local_1, local_1_tail, local_3);
...
napi_value js_carrier_sft_sft_open(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_sft_sft_open( local_0, local_1, local_1_tail, local_3);
    return jsreturn;
}
...
napi_value js_carrier_sft_sft_close(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_1 == 0 || status != napi_ok) {
        ...
        local_1 = tttt_local_1 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_sft_sft_close( local_0, local_1, local_1_tail);
    return jsreturn;
}


napi_value js_carrier_sft_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "sft_stream", NAPI_AUTO_LENGTH, js_carrier_sft_sft_stream, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "sft_stream", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "register", NAPI_AUTO_LENGTH, js_carrier_sft_register, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "register", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "sft_open", NAPI_AUTO_LENGTH, js_carrier_sft_sft_open, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "sft_open", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "sft_close", NAPI_AUTO_LENGTH, js_carrier_sft_sft_close, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "sft_close", ff);
    assert(status == napi_ok);
    return exports;
}
