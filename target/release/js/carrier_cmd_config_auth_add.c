...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_cmd_config_auth_add.h"
...
napi_value js_carrier_cmd_config_auth_add_run_remote(napi_env env, napi_callback_info info);
...
napi_value js_carrier_cmd_config_auth_add_run_self(napi_env env, napi_callback_info info);
...
napi_value js_carrier_cmd_config_auth_add_run_remote(napi_env env, napi_callback_info info) {
    ...
    carrier_cmd_config_Method  local_6;

                    if (status != napi_ok) {
                        napi_throw_type_error(env, 0, "6'th arg requires type ::carrier::cmd_config::Method");
                        return 0;
                    }
                    napi_value jsreturn = 0;
    carrier_cmd_config_auth_add_run_remote( local_0, local_0_tail, local_2, local_3, local_4, local_5, local_6);
    return jsreturn;
}
...
napi_value js_carrier_cmd_config_auth_add_run_self(napi_env env, napi_callback_info info) {
    ...
    carrier_cmd_config_Method  local_5;

                    if (status != napi_ok) {
                        napi_throw_type_error(env, 0, "5'th arg requires type ::carrier::cmd_config::Method");
                        return 0;
                    }
                    napi_value jsreturn = 0;
    carrier_cmd_config_auth_add_run_self( local_0, local_0_tail, local_2, local_3, local_4, local_5);
    return jsreturn;
}
...
napi_value js_carrier_cmd_config_auth_add_on_close(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_1 == 0 || status != napi_ok) {
        ...
        local_1 = tttt_local_1 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_cmd_config_auth_add_on_close( local_0, local_1, local_1_tail);
    return jsreturn;
}


napi_value js_carrier_cmd_config_auth_add_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "run_remote", NAPI_AUTO_LENGTH, js_carrier_cmd_config_auth_add_run_remote, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "run_remote", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "run_self", NAPI_AUTO_LENGTH, js_carrier_cmd_config_auth_add_run_self, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "run_self", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "on_close", NAPI_AUTO_LENGTH, js_carrier_cmd_config_auth_add_on_close, 0, &ff);
    assert(status == napi_ok);
