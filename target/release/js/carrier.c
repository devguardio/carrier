...
napi_value js_carrier_hellozz(napi_env env, napi_callback_info info);
...
napi_value js_carrier_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "hellozz", NAPI_AUTO_LENGTH, js_carrier_hellozz, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "hellozz", ff);
    assert(status == napi_ok);
    return exports;
}
