...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_cmd_common.h"
...
napi_value js_carrier_cmd_common_on_stream_want_header_200(napi_env env, napi_callback_info info) {
    ...
    bool  frrr = carrier_cmd_common_on_stream_want_header_200( local_0, local_1, local_1_tail, local_3);
...
napi_value js_carrier_cmd_common_print_identity(napi_env env, napi_callback_info info) {
    ...
    assert(status == napi_ok);
                    ...
                    napi_value jsreturn = 0;
    carrier_cmd_common_print_identity( local_0);
    return jsreturn;
}
...
napi_value js_carrier_cmd_common_on_printer_stream(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    bool  frrr = carrier_cmd_common_on_printer_stream( local_0, local_1, local_1_tail, local_3);
    status = napi_create_uint32(env, frrr, &jsreturn);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_carrier_cmd_common_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "on_stream_want_header_200", NAPI_AUTO_LENGTH, js_carrier_cmd_common_on_stream_want_header_200, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "on_stream_want_header_200", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "print_identity", NAPI_AUTO_LENGTH, js_carrier_cmd_common_print_identity, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "print_identity", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "on_printer_stream", NAPI_AUTO_LENGTH, js_carrier_cmd_common_on_printer_stream, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "on_printer_stream", ff);
    assert(status == napi_ok);
    return exports;
}
