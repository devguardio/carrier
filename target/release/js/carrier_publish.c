...
#include <stdint.h>
#include <string.h>
#include "zz/carrier/carrier_publish.h"
...
napi_value js_hex_dump_slice(napi_env env, napi_callback_info info);
...
napi_value js_carrier_publish_close_publish(napi_env env, napi_callback_info info) {
    ...
    carrier_publish_close_publish( local_0, local_1, local_1_tail);
    return jsreturn;
}
...
napi_value js_carrier_publish_on_remote_open(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_publish_on_remote_open( local_0, local_1, local_1_tail, local_3);
    return jsreturn;
}
...
napi_value js_carrier_publish_stream_to_publish(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    bool  frrr = carrier_publish_stream_to_publish( local_0, local_1, local_1_tail, local_3);
...
napi_value js_carrier_publish_stream_connect(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_3 == 0 || status != napi_ok) {
        ...
        local_3 = tttt_local_3 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    bool  frrr = carrier_publish_stream_connect( local_0, local_1, local_1_tail, local_3);
    status = napi_create_uint32(env, frrr, &jsreturn);
    assert(status == napi_ok);
    return jsreturn;
}
...
napi_value js_carrier_publish_publish(napi_env env, napi_callback_info info) {
    ...
    if (tttt_local_1 == 0 || status != napi_ok) {
        ...
        local_1 = tttt_local_1 + sizeof(size_t*);
                    ...
                    napi_value jsreturn = 0;
    carrier_publish_publish( local_0, local_1, local_1_tail);
    return jsreturn;
}


napi_value js_carrier_publish_Init(napi_env env, napi_value exports)
{
    napi_value ff;
    napi_status status;
    status = napi_create_function(env, "close_publish", NAPI_AUTO_LENGTH, js_carrier_publish_close_publish, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "close_publish", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "on_remote_open", NAPI_AUTO_LENGTH, js_carrier_publish_on_remote_open, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "on_remote_open", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "stream_to_publish", NAPI_AUTO_LENGTH, js_carrier_publish_stream_to_publish, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "stream_to_publish", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "stream_connect", NAPI_AUTO_LENGTH, js_carrier_publish_stream_connect, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "stream_connect", ff);
    assert(status == napi_ok);
    status = napi_create_function(env, "publish", NAPI_AUTO_LENGTH, js_carrier_publish_publish, 0, &ff);
    assert(status == napi_ok);
    status = napi_set_named_property(env, exports, "publish", ff);
    assert(status == napi_ok);
    return exports;
}
