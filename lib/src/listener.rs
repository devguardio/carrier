...
pub fn listen(secret: identity::Secret) -> Result<(Listener, shadow::broker::Handle), Error> {
    let stdsock = StdSocket::bind(&format!("0.0.0.0:{}", env::var("PORT").unwrap_or("8443".to_string())))?;
    ...
    work.try_send(endpoint::EndpointWorkerCmd::InsertChannel(0, endpoint::ChannelBus::User{inc:tx}))?;
...
impl ChannelHandshake {
    ...
    pub fn accept(self, secret: identity::Secret) -> impl Future<Item=channel::Channel, Error=Error> {
        ...
        self.work.send(endpoint::EndpointWorkerCmd::AquireChannel(route_tx, bus))
            .map_err(Error::from)
            .and_then(move |work|{
