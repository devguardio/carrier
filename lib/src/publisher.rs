...
pub fn dispatch<F>(
    ...
    where F: 'static + FnMut(&identity::Identity) -> bool + Send + Sync
{
    let (xsecret, xpublic) = identity::generate_x25519();
    let xaddr = identity::SignedAddress::sign(&secret, identity::Address(xpublic));

    let publish_change = proto::Broker::publish(&mut brk, proto::PublishRequest{
    ...
    let never = publish_change.select(proto::Peer::dispatch(brk.listener().unwrap(), publisher))
        .map_err(|(e,_)|warn!("{}", e))
        .and_then(|_|{
            drop(brk);
            Ok(())
