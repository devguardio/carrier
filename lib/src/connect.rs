...
pub fn connect<S: AsRef<str>> (domain: S, secret: identity::Secret) -> impl Future<Item = (Endpoint, Channel, StdSocket, SocketAddr), Error = Error> {
    dns::resolve(domain.as_ref()).and_then(|records|{
...
impl Future for EndpointFuture {
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        ...
        match st {
            ...
            EndpointFutureState::WaitingForResponse{stdsock, mut miosock, mut noise, pkt, mut deadline, mut attempts, addr} => {
                ...
                let (len, addr) = match miosock.poll_recv_from(&mut buf)? {
                    Async::Ready(v) => v,
                    Async::NotReady => {
                        self.st = EndpointFutureState::WaitingForResponse{stdsock, miosock, noise, pkt, deadline, attempts, addr};
                        return Ok(Async::NotReady);
                    }
                };

                match EncryptedPacket::decode(&buf[..len]).and_then(|pkt|{
                    noise.recv_response(pkt)
                }){
                    Ok(identity) => {
                        ...
                        ep.work.try_send(endpoint::EndpointWorkerCmd::InsertChannel(noise.route, endpoint::ChannelBus::User{inc:tx}))?;
